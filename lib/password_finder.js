...
function printOutUserReport(user, result) {
    console.log('------------------------------------------------');
    _.isObject(user) ? console.log(`Report for user ${user.login} with ID ${user.id}\r\n`) : console.log(`Report for user ${user}\r\n`);
    _.isEmpty(result.repositoryFiles) && _.isEmpty(result.commitFiles) ? console.info('User is clean') : console.log(JSON.stringify(result, null, '\t'));
}
    ...
    findAll: async since => {
        ...
        while(users.length > 0) {
            for (let user of users) {
                try {
                    await findForUser(user.login, user.id);
                } catch(err) {
                   console.error(err);
                }
            }
