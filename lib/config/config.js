...
async function initDb() {
    if (!options.opt.mongo || !options.opt.all) {
        return;
    }
    const since = await repository.init(options.opt.mongo);
    if (_.isEmpty(module.exports.opts.since)) {
        module.exports.opts.since = since;
    }
}
