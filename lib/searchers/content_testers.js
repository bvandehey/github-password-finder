...
const TESTERS = [ //TODO: add testers for eclipse and intelliJ configuration files
    ...
    buildTester({
        ...
        getVariableByNameKeyword: (keyword, content) => content.match(new RegExp(`string [ \ta-zA-Z0-9_ ]*(${keyword})[a-zA-Z0-9_ \t]*=[ \t]*"(${PASSWORD_ALLOWED_CHARACTERS}+)"`, 'ig')) || content.match(new RegExp(`\\.(set|with)[a-zA-Z_]*(${keyword})\\("(.+)"\\)`, 'ig')),
        getVariableByValue: (value, content) => content.match(new RegExp(`.*".*${value}.*".*`, 'ig'))
    }),
    buildTester({
    ...
    buildTester({
        extensions: ['xml'],
        getVariableByNameKeyword: (keyword, content) => content.match(new RegExp(`\\<.*(${keyword}).*\\>${PASSWORD_ALLOWED_CHARACTERS}+\\<\\/.*(${keyword}).*\\>`, 'ig')) || content.match(new RegExp(`.*(${keyword})[a-zA-Z_\\-]*="${PASSWORD_ALLOWED_CHARACTERS}+"`, 'ig')),
